...
def browser_init() -> webdriver:
	...
	opts = Options()
	# opts.headless = True  # TODO uncomment when finished
...
def find_object(web_browser: webdriver, xpath: XPath):
	"""Wrapper for find_element Selenium method"""
	return web_browser.find_element(By.XPATH, xpath)


def find_multiple_objects(web_browser: webdriver, xpath: XPath):
	"""Wrapper for find_elements Selenium method"""
	return web_browser.find_elements(By.XPATH, xpath)


def wait(web_browser: webdriver, method: WaitMethod, xpath: XPath, timeout: int = 20, msg: str = "Error"):
...
def click_element(web_browser: webdriver, xpath: XPath, timeout: int = 10) -> bool:
...
def hover_on_element(web_browser: webdriver, xpath: XPath) -> None:
	"""Finds an element by XPath and hovers over it"""
	try:
		WebDriverWait(web_browser, 20).until(ec.element_to_be_clickable((By.XPATH, xpath)),
